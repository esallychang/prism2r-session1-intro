...
users to carry out their analyses in the cloud. There are advantages to
...
are teaching you R using Posit Cloud? And the answer is that Posit Cloud
offers a great environment for teaching from and all of the principles
that we learn using R and RStudio on Posit Cloud will apply to your
local installation of R and RStudio as well. Some of these benefits
include:
...
Our recommendation is for you to familiarize yourself with R and RStudio
in this workshop in a common shared environment on Posit Cloud and then
...
have not done so already. Once you have made an account on Posit Cloud,
the instructors for the workshop will have shared a link with you to
join the Introduction to R workspace hosted by the Harvard Chan
Bioinformatics Core or you can click [this
link](https://posit.cloud/spaces/658857/join?access_code=gtJO2BXo7o0poV0uLxVEi_2ZKgYpUqoisHP7UZBf).
This will take you to a webpage that looks like this:

<img src="../img/Join_workspace_annotated.png" width="600"/>

Once you hit the "Join" Button and it will take you to a webpage that
...
In the left-hand panel you should see a section called "Spaces". Within
it you should see three options:
...
It is simply a directory (or folder) that contains everything related
your analyses for a specific project. RStudio projects are useful when
you are working on context-specific analyses and you wish to keep them
...
A `. RProj file` is created within that directory and that keeps track
of your command history and variables in the environment. The
`. RProj file` can be used to open the project in its current state but
at a later date.
...
We have already begun setting up the project for you and you should see
...
::: callout-note
Once you have opened the "Intro-to-R" on Posit Cloud for the first time,
it will make a derived duplicate of the project that you are working on
in our **Introduction to R** Workspace. In the future, you can click on
either R project to come back to the point where you left off.
:::
...
the four files that we will be working with.

<img src="../img/Data_files_annotated.png" width="500"/>

::: callout-note
If you wanted to add files onto the Posit Cloud server you could do this
with the "Upload" button in the "Files" tab.
:::
...
We will be using the results from a functional analysis to learn about
